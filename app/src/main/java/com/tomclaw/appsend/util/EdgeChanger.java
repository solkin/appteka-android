...
public class EdgeChanger {
    ...
    private static void invalidateNestedScrollViewFields() {
        ...
        final Class<?> cls = inClassPath(NESTEDSCROLLVIEW_CLASS);
        for (Field f : cls.getDeclaredFields()) {
            switch (f.getName()) {
                case "mEdgeGlowTop":
                    f.setAccessible(true);
    ...
    private static void invalidateRecyclerViewFields() {
        ...
        final Class<?> cls = inClassPath(RECYCLERVIEW_CLASS);
        for (Field f : cls.getDeclaredFields()) {
            switch (f.getName()) {
                case "mTopGlow":
                    f.setAccessible(true);
    ...
    private static void invalidateViewPagerFields() {
        ...
        final Class<?> cls = inClassPath(VIEWPAGER_CLASS);
        for (Field f : cls.getDeclaredFields()) {
            switch (f.getName()) {
                case "mLeftEdge":
                    f.setAccessible(true);
        ...
        if (scrollListener == null) {
            scrollListener = new RecyclerView.OnScrollListener() {
                ...
                public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                    ...
                    EdgeChanger.setEdgeGlowColor(recyclerView, color, this);
                }
            };
